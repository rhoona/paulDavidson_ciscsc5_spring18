...
//System Libraries
#include <iostream>
//#include <string> did not have time to implement after finishing program =(
#include <cstdlib>
#include <ctime>
using namespace std;
...
void rowStar() //function for stars(pure cosmetic/seperator)
{
cout<<"*****************************************\n"<<endl;
}
int main(int argc, char** argv) {
    ...
    //roll function returns the value for roll
      roll = ROLL(p1Skip,p2Skip,p1Turn,p2Turn);
  ...
  */   
    ...
    rowStar();//simple calls the rowstar decoration function
 ...
 */
    ...
    int ROLL(int p1skip,int p2Skip,int p1Turn,int p2Turn)
    {
    int ROLL = 0;
    char input = 0;
    int i = 0;
    ...
    {
    cout<<"P2[COMPUTER] used a Get out of jail Free card! P2[COMPUTER] is moved to\n"
    ...
    {
    card5(ifOpen,posVal,p1,p2,p1Yoki,p2Yoki,p1Pos,p2Pos,p1Skip,p2Skip,p1Money,p2Money);
    }
    }
    ...
    "While P2[COMPUTER]'S HIGHEST valued card was at #"<<SIZE<<" worth "<<posVal[SIZE]<<"\n"
    "so you traded P2[COMPUTER]'s $"<<posVal[SIZE]<<" Yoki for your $"<<posVal[size]<<" Yoki!\n";
    ...
    "P2[COMPUTER] highest valued Yoki is only worth $"<<posVal[SIZE]<<" on #"<<SIZE<<"\n"
    ...
    ifOpen[size] = 4;
    ifOpen[SIZE] = 3;
    }else  if(p1 && ifOpen[size] == 3 && posVal[size] == posVal[SIZE] && p1Skip == 0)
    {
    cout<<"UGGH!!! !!!\n"
    ...
    "Usuarly this card allows you to swap your lowest Yoki with P2[COMPUTER]'S HIGHEST!!\n"
    ...
    "P2[COMPUTER] took your $"<<posVal[SIZE]<<" Yoki for his "<<posVal[size]<<" Yoki!\n";
    ifOpen[size] = 3;
    ifOpen[SIZE] = 4;
    ...
    {
    cout<<"LOOOL! P2[COMPUTER] would have swaped you  his lowest valued Yoki for yout highest but\n"
   ...
   {
   cout<< "UNFORTUNATLY FOR P2[COMPUTER] YOUR LOWEST VALUED YOKI IS THE SAME AS HIS highest!!!:(!\n"
    "P2[COMPUTER]'s lowest Valued yoki is on #"<<size<<" worth $"<<posVal[size]<<"\n"
    ...
    {
    cout<<"!!!! alert !!!!\n";
    cout<<"P2[COMPUTER] LANDED ON THE ""CHANCE"" CARD to swap yoki but you had [0] SO you lose a turn!\n";
    p1Skip = 2;
    }
    }
    ...
    {        
    //possible issue when no open cards, but probably not as game should have ended by that point!
    ...
    if(ifOpen[p1Pos] == 1)
    ifOpen[p1Pos] = 3;
    cout<<"You got the Yoki at #"<<p1Pos<<" For free!, it's value is "<<posVal[p1Pos]<<endl;
     p1Yoki = p1Yoki + 1;
    ...
    if(ifOpen[p2Pos] == 1)
    ifOpen[p2Pos] = 4;
    cout<<"P2[COMPUTER] whent to #"<<p2Pos<< " and recieved the Yoki there worth $"<<
    ...
    {
    cout<<"OHH INteresting! You received a ""Get out of Jail free card! IM sure you know whats that for!\n";
    p1Jfc = p1Jfc + 1;
    ...
    {
    cout<<"P2[COMPUTER] DREW A ""GET OUT OF JAIL FREE CARD"" But already has the max of 2/2\n"
