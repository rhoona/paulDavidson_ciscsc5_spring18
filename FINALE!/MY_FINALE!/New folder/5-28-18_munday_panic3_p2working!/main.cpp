...
int mustPay( int *,int&,int&,int,int,bool,bool );
void rowStar()
{
cout<<"*****************************************\n"<<endl;
}
int main(int argc, char** argv) {
    ...
    int ifOpen[24] = {0,4,4,0,4,4,0,4,4,0,4,4,0,4,4,0,4,4,0,4,4,0,4,4};
    int *IO = ifOpen;
    
    int p1Money = 300,p2Money = 3;
...
//p1 turn is true so he rolls, after his turn = false and p2 turn is true;
    ...
    } else if (ifOpen[p1Position] == 4 && p1Money >= positionsValue[p1Position])
    {
        mustPay(pV,p1Money,p2Money,p1Position,p2Position,p1Turn,p2Turn);
    }
  ...
  */   
    ...
    p2Money = p2Money + positionsValue[0];
     ...
     }else if(ifOpen[p2Position] == 3 && p2Money < positionsValue[p2Position] && p2Yoki == 0)
     {
...
int mustPay(int *posValue, int & p1Money,int & p2Money,int p1Pos, int p2Pos, bool p1Turn, bool p2Turn)
{
    //WORKING OMG!!
